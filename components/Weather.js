...
class MarsDate {
  ...
  formatDate(s) {
    ...
    s = s.replace(
      'Z',
      MarsDate.i18n.timezones['+0000'].tz + a.Z.offset + ' (' + a.Z.tz + ')'
    );
  ...
  */
  ...
  getMSDFromDate(x) {
    var m =
      ...
      (x.m - 1) * 28 -
      Math.floor((x.m - 1) / 6) +
      (x.l + 1000 * (x.s + 60 * (x.M + 60 * x.H))) / 86400000;
    for (var i = 0; i < x.y; i++) {
    ...
    return (
      <li key={sol.id} className="border rounded-md p-4">
        <ul>
          ...
          <li>
            Mars date:
            <span> {new MarsDate(new Date(sol.terrestrial_date)).string.slice(0,-24)}</span>
          </li>
          {/* <li>
